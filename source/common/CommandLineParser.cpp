...
static bool parseCommandLine(const vector<option>& options, int argc, char** argv)
{
	...
	while(true)
	{
		int c = getopt_long(argc, argv, "hm:s:p:l:", &options[0], &options_index);
		if (c == -1)
			break;
		cout << "c:" << char(c) << endl;
		switch(c)
		{
			...
			case 's':
				...
				break;
			case 'p':
				GlobalVariable::instance().option()->setServerPort(atoi(optarg));
				break;
			case 'l':
				GlobalVariable::instance().option()->setLocalPort(atoi(optarg));
				break;
			case 'k':
				GlobalVariable::instance().option()->setPassword(optarg);
				break;
			case 't':
				GlobalVariable::instance().option()->setTimeout(atoi(optarg));
				break;
			case 'c':
				JsonParser::parse(optarg);
		}
	}
}
